                        ...
                        <td valign="top">
                            <table>
                                ...
                                <ctl:Text ID="txtDispoDate" runat="server" Label="Dispo Date:" Required="false" Width="140"
                                    ValidationType="Date" />
                                <ctl:Text ID="txtDOBFound" runat="server" Label="DOB Found:" Width="140" ValidationType="Date" Required="true" />
                                <ctl:Text ID="txtViolationDate" runat="server" Label="Violation Date:" Required="false" Width="140" ValidationType="Date" />
                                <ctl:Text ID="txtDistrictCourt" runat="server" Label="District Court:" Required="false" Width="140" />
                                <ctl:Text ID="txtCaseTypeLevel" runat="server" Label="Case Type Level:" Required="false" Width="140" />
                                <ctl:Text ID="txtDefendant" runat="server" Label="Defendant:" Required="false" Width="140" />
                                <ctl:Text ID="txtStatusJudgment" runat="server" Label="Status/Judgment:" Required="false" Width="140" />
                            </table>
                        </td>
                        <td  valign="top">
                            <table>
                    ...
                    <tr>
                        <td colspan="4">
                            <asp:UpdatePanel ID="upCaseRecord" runat="server" UpdateMode="Conditional">
                                <ContentTemplate>
                                    <asp:GridView ID="grvCaseRecord" runat="server" ShowFooter="false" DataKeyNames="ID" AutoGenerateColumns="false" OnRowCancelingEdit="OnCancel" 
                                    OnRowDeleting="OnDelete" OnRowEditing="OnEdit" OnRowUpdating="OnUpdate" HeaderStyle-CssClass="GridHeader" AlternatingRowStyle-CssClass="GridAltItem">
                                        <Columns>
                                            ...
                                            <asp:TemplateField ItemStyle-Width="192px" HeaderText="Description">
                                                ...
                                                <EditItemTemplate>
                                                    <ctl:Text2 ID="txtDescription" runat="server" Value='<%# Bind("Description") %>' Width="180"  Required="true" ValidationMessage="Description" />
                                                </EditItemTemplate>
                                            </asp:TemplateField>
                                            <asp:TemplateField ItemStyle-Width="48px" HeaderText="Level">
                                                <ItemTemplate>
                                                    <ctl:DDown ID="ddlLevelRead" runat="server" Width="36" DataTextField="Name" DataValueField="ID" Enabled="false" DataSource='<%# Bind("DSLevelTypeID") %>' Value='<%# Bind("LevelTypeID") %>' />
                                                </ItemTemplate>
                                                <EditItemTemplate>
                                                    <ctl:DDown ID="ddlLevel" runat="server" Width="36" DataTextField="Name" DataValueField="ID" DataSource='<%# Bind("DSLevelTypeID") %>' Value='<%# Bind("LevelTypeID") %>' />
                                                </EditItemTemplate>
                                            </asp:TemplateField>
                                            <asp:TemplateField ItemStyle-Width="48px" HeaderText="Plea">
                                                <ItemTemplate>
                                                    <ctl:DDown ID="ddlPleaRead" runat="server" Width="36" DataTextField="Name" DataValueField="ID" Enabled="false" DataSource='<%# Bind("DSPleaTypeID") %>' Value='<%# Bind("PleaTypeID") %>' />
                                                </ItemTemplate>
                                                <EditItemTemplate>
                                                    <ctl:DDown ID="ddlPlea" runat="server" Width="36" DataTextField="Name" DataValueField="ID" DataSource='<%# Bind("DSPleaTypeID") %>' Value='<%# Bind("PleaTypeID") %>' />
                                                </EditItemTemplate>
                                            </asp:TemplateField>
                                            <asp:TemplateField ItemStyle-Width="48px" HeaderText="Disposition">
                                                <ItemTemplate>
                                                    <ctl:DDown ID="ddlDispositionRead" runat="server" Width="36" DataTextField="Name" DataValueField="ID" Enabled="false" DataSource='<%# Bind("DSDispositionTypeID") %>' Value='<%# Bind("DispositionTypeID") %>' />
                                                </ItemTemplate>
                                                <EditItemTemplate>
                                                    <ctl:DDown ID="ddlDisposition" runat="server" Width="36" DataTextField="Name" DataValueField="ID" DataSource='<%# Bind("DSDispositionTypeID") %>' Value='<%# Bind("DispositionTypeID") %>' />
                                                </EditItemTemplate>
                                            </asp:TemplateField>
                                            <asp:CommandField ShowEditButton="true" ItemStyle-Width="80px" />
                                             <asp:TemplateField ItemStyle-Width="48px">
                                                <ItemTemplate>
                                                    <asp:LinkButton ID="lnkDelete" runat="server" OnClientClick="return confirm('Are you sure to delete this case?');" CausesValidation="false" Visible = '<%# Eval("ID") == null ? false:true %>'
