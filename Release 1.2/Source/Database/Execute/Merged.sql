...
SET @SqlString	=  @SqlString + ' WHERE ' + @SearchData    
SET @SqlString	=  @SqlString + ' GROUP BY B.ID,C.Name, B.ReceivedDate, B.CreatedDate, B.CustomerRefNo,RS.Name,CY.Name,B.Name,B.DOB,CS.Name,B.Years,A.Name,B.ClientRemarks,B.ClientSplRemarks, CY.Name'
SET @SqlString	=  @SqlString + ' ORDER BY ' + @SortField   
...
BEGIN
	...
	GROUP BY R.ID, RecordID,R.CreatedDate,R.ModifiedDate,R.NameFound,R.DOBFound,R.Alias,R.CaseNumber,R.ViolationDate,R.FileDate,R.DispoDate,R.ProbationType,R.FileFee,R.Restitution,R.Jail,R.JailCredit,R.SentencingDetail,	R.DistrictCourt, R.CaseTypeLevel, R.CaseName,	R.Plaintiff,	R.Defendant,	R.StatusJudgment, CT.IsCivil
...
DECLARE @strName VARCHAR(2000),@ind Int,@ind1 Int, @sep VARCHAR(5),@sep1 VARCHAR(5), @sep2 VARCHAR(5), @strFName VARCHAR(200), @strLName VARCHAR(200)
DECLARE @strQry nvarchar(4000), @strWhere varchar(4000)

select @sep = '~', @sep1 =',', @sep2 =' '

SELECT @NameSent = br.Name, @CountyID = CountyID, @DOB=br.DOB ,@CaseTypeID = br.CaseTypeID  
