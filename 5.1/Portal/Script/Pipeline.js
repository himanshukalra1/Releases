...
function SelectAll(obj) {
    ...
    for (var rowNum = 0; rowNum < table.rows.length - 1; rowNum++) {
        if (rowNum > 0) {
            if (table.rows[rowNum].children[17].children[0].style.visibility == 'hidden') //spl remarks
...
function ValidateStatus() {
    var table = document.getElementById('cphMaster_grvRecord');
    var toStatus = document.getElementById('cphMaster_ddlChangeStatus_ucDDL').value;
    if (toStatus == 5)//ARCHIVE
    {
        for (var rowNum = 0; rowNum < table.rows.length - 1; rowNum++) {
            if (rowNum > 0) {
                var status = table.rows[rowNum].children[6].innerText;
                if (status.indexOf('Auto') > -1) {
                    alert("State transition not allowed from Auto Hit/Auto Clear to ARCHIVE.")
                    return false;
                }
            }
        }
