...
function setfocus(ctlid) { $('#' + ctlid).focus(); }
...
/*-------------------------------------------------------------------------------------------------------------------------------------------*/
/* To handle text-box */
function focus2(ctlid, isdecimal) {
    var ctl = document.getElementById(ctlid);
    if (isdecimal) ctl.innerText = ctl.value.replace(/,/g, '');
    ctl.select();
}

function onkeypress(ctl, rx, maxlen, isdec,evt) {
    ...
    return (eval(rx).test(String.fromCharCode(((evt.which) ? evt.which : evt.keyCode))) && ismaxlenvalidation);
}
...
function disableme(obj, callfunction) {
    ...
    if (!isproceed) {//handling the validation error messages and the control focus on the click of the error message links
        setdisplay(tdinfoid);
        var objvs = getelement(Page_ValidationSummaries[0].id);
        if (objvs != null) {
            var objpv = null, childNodes = objvs.childNodes[1].childNodes, ctl2v = null; //first child (objvs.childNodes[0]) will be displaying the header text
            for (ix = 0; ix < Page_Validators.length; ix++) {
                objpv = $(getelement(Page_Validators[ix].id));
                ctl2v = objpv.prop(ctlvalidate);
                for (iy = 0; iy < childNodes.length; iy++) {
                    if ((childNodes[iy].innerText != undefined && childNodes[iy].innerText == objpv.prop(ctlerrormsg)) ||
                        (childNodes[iy].innerText == undefined && childNodes[iy].innerHTML == objpv.prop(ctlerrormsg))) {
                        childNodes[iy].innerHTML = "<a class='Error' href='#' onclick='setfocus("
                                + doublequote + ctl2v + doublequote + ")'>" + $(childNodes[iy]).text() + "</a>";
                        break;
                    }
                }
            }
    ...
    if (obj != null && isproceed) {//when the instance of the button is available and all the validators have return the result as true or there is no validators available in the page
        obj.disabled = true;
...
function getffcontrol(ctl, pv) {
    return ($('#' + pv.prop(ctlvalidate)).closest('td').prev('td').find('span').text() == ctl.innerHTML);
}
...
function setdate(ctlid, date1st) {
    var options = {};
    if (date1st) options.beforeShowDay = function (date) {
        if (date.getDate() == 1) {
            return [true, ''];
        }
        return [false, ''];
    };
    var pick = $('#' + ctlid).datepicker(options);
    pick.datepicker('setDate', pick.val()).blur(formatdate).keydown(function () { $(this).datepicker('hide'); });
}

function formatdate() {
    var cdate = $(this);
    if (!cdate.datepicker('widget').is(':visible'))
        cdate.datepicker('setDate', cdate.val());
}
